...
- If the first live self-enforcement canary proves that the protected `pull_request_target` caller cannot execute its own corrective PR, the still-open bootstrap transaction permits one corrective bootstrap PR. It must repair only reproduced bootstrap defects, satisfy the same exact-SHA verification and rollback requirements, use only the same `enforce_admins` toggle, and restore protection immediately. Produce one schema-valid audit receipt per merge cycle so each receipt binds its own candidate, pull request, merge, rollback, mutation pair, and protection snapshots; final proof must list both content hashes. This authority expires with the bootstrap transaction and cannot be used for later maintenance.
