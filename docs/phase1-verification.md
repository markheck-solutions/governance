...
Delivery readiness requires the benchmark JSON result, not only a green GitHub status context. The accepted result must validate against the benchmark schema, contain `phase1_decision=BENCHMARK_PASS`, empty `acceptance_errors`, a recomputable `artifact_content_hash`, case counts, and all required metric numerators, denominators, stability fields, and flake fields. When readiness is evaluated against GitHub artifact evidence, the GitHub artifact digest must also be supplied.

The final review gate is recorded as `review_gate=GITHUB_CODEX_FINAL_REVIEW|FALLBACK_CLEAN_ROOM_QUORUM` with `github_review_state=CLEAN|STALE|UNAVAILABLE|BLOCKING_FINDINGS_PRESENT`. If GitHub Codex review is stale or unavailable, a fallback clean-room quorum can be supplied as schema-valid JSON under `artifacts/review-quorum/`; delivery readiness requires exact top-level and reviewer base/head SHAs plus clean-room reviewer provenance. The shadow workflow validates supplied quorum JSON before uploading it as `governance-review-quorum-json`.
