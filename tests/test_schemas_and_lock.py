...
from governance_eval.lock import read_target_lock, validate_target_lock, write_target_lock
...
class SchemaAndLockTests(unittest.TestCase):
    ...
    def test_core_has_no_target_specific_identity_and_registry_has_command_adapter(self) -> None:
        violations = []
        for path in sorted((self.root / "governance_eval").glob("*.py")):
            if "spaghetti" in path.read_text(encoding="utf-8").lower():
                violations.append(path.name)
        registry = load_target_registry(root=self.root)
        pack_paths = {entry["path"] for entry in registry["packs"]}
    ...
    def test_registered_target_lock_contains_full_immutable_shas(self) -> None:
        lock_path = self.root / "targets/spaghetti.lock.toml"
        pack_path = self.root / "target_packs/spaghetti/v1/pack.json"
        self.assertEqual(validate_target_lock(lock_path, pack_path, root=self.root), [])
        lock = read_target_lock(lock_path)
        for key, value in lock.revisions.items():
            if key.endswith("_sha"):
                self.assertRegex(value, r"^[0-9a-f]{40}$")
        self.assertEqual(lock.metadata["pull_request"], 141)

    def test_lock_writer_generates_schema_versioned_toml(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "target.lock.toml"
            write_target_lock(
                path,
                self.root / "target_packs/spaghetti/v1/pack.json",
                evidence_source="test evidence",
                metadata={"pull_request": 141},
                root=self.root,
            )
            self.assertTrue(path.exists())
            self.assertEqual(
                validate_target_lock(path, self.root / "target_packs/spaghetti/v1/pack.json", root=self.root), []
            )
