...
def load_target_pack(path: Path, root: Path | None = None, require_governance_owned: bool = False) -> dict[str, Any]:
    governance_root = repo_root(root or Path(__file__).resolve())
    resolved = resolve_pack_path(path, governance_root, require_governance_owned=require_governance_owned)
...
def resolve_pack_path(path: Path, root: Path, require_governance_owned: bool = False) -> Path:
    if not path.is_absolute():
        if any(part == ".." for part in path.parts):
            raise SchemaValidationError(f"target pack path traversal is not allowed: {path}")
    ...
    if require_governance_owned or not path.is_absolute():
        try:
            ...
            raise SchemaValidationError(f"target pack must stay inside governance repository: {path}") from exc
...
def _validate_detector_policies(pack: dict[str, Any]) -> None:
    ...
    if missing:
        raise SchemaValidationError(f"{pack['id']}: detector_policies missing {missing}")
    for detector, policy in policies.items():
        for field in ("required", "blocking", "fail_on_unknown"):
            if field not in policy:
                raise SchemaValidationError(f"{pack['id']}: detector_policies.{detector}.{field} required")
...
def _validate_revision_modes(pack: dict[str, Any]) -> None:
    ...
    if not required.issubset(supported):
        raise SchemaValidationError(f"{pack['id']}: revision_modes must include {sorted(required)}")
    revisions = pack["immutable_revisions"]
    for field in ("base_sha", "head_sha"):
        if not SHA_RE.fullmatch(revisions[field]):
            raise SchemaValidationError(f"{pack['id']}: immutable_revisions.{field} must be a full SHA")
...
def _validate_behavior_cases(pack: dict[str, Any], root: Path) -> None:
    for case in pack["behavior_cases"]:
        ...
        if "PINNED_EXPECTED" in policy.values() and "expected_base_result" not in case:
            raise SchemaValidationError(f"{case['id']}: expected_base_result required for PINNED_EXPECTED")
...
def validate_target_request(
    ...
    governance_root = repo_root(root or Path(__file__).resolve())
    pack = load_target_pack(pack_path, root=governance_root, require_governance_owned=True)
...
def _validate_requested_revisions(
    ...
    if revision_mode not in set(pack.get("revision_modes", [])):
        raise ValueError(f"revision mode is not supported by pack {pack['id']}: {revision_mode}")
...
def infer_revision_mode(pack: dict[str, Any], base_sha: str, head_sha: str, merge_sha: str | None) -> str:
    ...
    if base_sha == revisions.get("safe_base_sha") and head_sha == revisions.get("safe_head_sha") and not merge_sha:
        return "SAFE_FIXED"
